"""
    @Time: 2018/5/10 14:07
    @Author: qingyaocui
"""
import os
import pickle
from course.lib import utils
...
class Nid:
    def __init__(self, role, db_path):
        ...
        :param db_path:
        '''
        role_set = {'admin', 'teacher', 'classes', 'student',
                    'school', 'course', 'course_to_teacher'}
        if role not in role_set:
            raise Exception('用户角色定义错误, 选项为：%s' % (','.join(role_set)))
        self.role = role
